...
def gcd_coprime(a, b)
  while b != 0
    temp = a       # Сохраняем текущее значение a во временную переменную
a = b          # Меняем местами: a получает значение b
b = temp % b   # b заменяем на остаток от деления предыдущего a на b
  end
  a == 1
end
...
while true
  ...
  # Проверяем на выход из цикла
  break if user_input == "0"

  # Преобразуем выбор в ключ, если это число
